...
class OPENBRIDGE(DatagramProtocol):
    ...
    def datagramReceived(self, _packet, _sockaddr):
        ...
        if _packet[:4] == DMRD:    # DMRData -- encapsulated DMR data frame
           ...
           logger.debug('(%s) TalkerAlias, packet discarded - OPCODE: %s DATA: %s HMAC LENGTH: %s HMAC: %s', self._system, _packet[:4], repr(_packet[:53]), len(_packet[53:]), repr(_packet[53:]))
        elif _packet[:4] == DMRG:
           logger.debug('(%s) GPS, packet discarded - OPCODE: %s DATA: %s HMAC LENGTH: %s HMAC: %s', self._system, _packet[:4], repr(_packet[:53]), len(_packet[53:]), repr(_packet[53:]))
